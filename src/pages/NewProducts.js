...
const NewProducts = ({ setChange }) => {
    ...
    if (id) {
      fetch(`http://vorn.ponlue.bio:5046/api/product/${id}`)
        ...
        .then((res) => {
          ...
          const imgurl= res.image_id.map((img) => img.files);
          const uploadimg = res.image_id.map((img)=>img.id);
          setProduct(res);
          setImage(imgid);
          setImageurl(imgurl)
  ...
  let buttonStatus =
    name && stockqty && price && category_id && description && images
      ? false
      : true;
    ...
    <div className="container ">
      <div className="row  bg-light rounded-4  p-4 " style={{ marginTop: 50 }}>
        <div className="col-lg-6 col-xs-12">
          ...
          <img src={`http://vorn.ponlue.bio:5046${imagesurl[selectedIndex]}`}  alt="imageurl" className='object-fit-contain img-fluid  rounded-3' width='350px' />
              <div className="ImgCard">
                  {
                    imagesurl.map((image,id)=>
                      <div className="image">
                        <span className="delete" onClick={()=>deleteImage(id)}>&times;</span>
                        <img src={`http://vorn.ponlue.bio:5046${image}`} alt={`Image ${id}`} onClick={()=>handleclick(id)}/>
                      </div>
                    )
                  }
                  {preview.map((url, id) => (
        ...
        <div className="col-lg-6 col-xs-12">
          <div className="input-side">
            ...
            <div className="AllButton d-flex justify-content-center mt-5">
              ...
              <button className="btn btn-danger ms-5 " onClick={handleClear}>
                Cancel
              </button>
            </div>
            <ToastContainer />
